...
3. **初回起動前に**、ターミナルで以下を実行（Gatekeeper 回避）:
   ```bash
   xattr -cr /Applications/LLMime.app
   ```
   または、右クリック →「開く」→「開く」で起動することもできます。
4. 起動するとメニューバーにアイコンが表示される
5. 「設定…」から Gemini API キーを入力

> **なぜこの手順が必要？**
> LLMime は Apple Developer ID（年間$99）で署名されていないため、macOS Gatekeeper が未検証アプリとして警告を表示します。上記の手順で安全に起動できます。ソースコードは本リポジトリで全て公開されています。
